...
--
-- Multi-app roles: one canonical user can use both mobile applications.
--
CREATE TABLE `user_app_roles` (
  `user_id` bigint UNSIGNED NOT NULL,
  `app_role` tinyint UNSIGNED NOT NULL COMMENT '1=Driver, 2=Cargo/Company',
  `status` tinyint UNSIGNED NOT NULL DEFAULT 1 COMMENT '1=Active, 2=Disabled',
  `created_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updated_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`user_id`,`app_role`),
  KEY `idx_user_app_roles_role_status` (`app_role`,`status`),
...
SELECT `id`, `user_type`, 1, NOW(3), NOW(3) FROM `users` WHERE `user_type` IN (1,2);

--
-- FAQ content, separated by target application.
--
CREATE TABLE `faq_categories` (
  `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
  `target_app_id` tinyint UNSIGNED NOT NULL COMMENT '1=Driver, 2=Cargo',
  ...
  `updated_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
  KEY `idx_faq_categories_target_active_sort` (`target_app_id`,`is_active`,`sort_order`),
  KEY `idx_faq_categories_creator` (`created_by_user_id`),
...
CREATE TABLE `faq_items` (
  ...
  `updated_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
  KEY `idx_faq_items_category_active_sort` (`category_id`,`is_active`,`sort_order`),
  KEY `idx_faq_items_creator` (`created_by_user_id`),

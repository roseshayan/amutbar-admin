...
if ($action === 'reply') {
    ...
    if (!$ticketId || !$message) {
        echo json_encode(['ok' => false, 'message' => 'متن پیام الزامی است']);
        exit;
    }

    $pdo->beginTransaction();
    try {
        // دریافت آیدی کاربر برای ارسال نوتیف
        ...
        $pdo->prepare("UPDATE support_tickets SET status=2, updated_at=NOW(3) WHERE id=?")->execute([$ticketId]);

        // --- ثبت نوتیفیکیشن هوشمند برای راننده ---
        if ($targetUserId) {
            // جاسازی آیدی تیکت در دیتای نوتیفیکیشن
            $dataJson = json_encode(['ticket_id' => $ticketId], JSON_UNESCAPED_UNICODE);

            $pdo->prepare("INSERT INTO notifications (user_id, type, title, body, data_json, created_at) VALUES (?, 'ticket_reply', 'پاسخ به تیکت', 'پشتیبانی به تیکت شما پاسخ داد.', ?, NOW(3))")
